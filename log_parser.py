...
def _insert_threat(timestamp: str, source_ip: str, event_type: str, raw_log: str, ai_analysis: dict) -> None:
    """Inserts a threat record into the database."""
    ...
    with db_init.get_connection() as conn:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (timestamp, source_ip, event_type, raw_log, severity, explanation, recommendation, False))
        conn.commit()

    log.info(f"[{severity}] {event_type} from {source_ip}: {explanation}")
            ...
            for line in lines:
                ...
                for event_type, pattern in patterns.items():
                    ...
                    if match:
                        ...
                        threats_found.append({"event_type": event_type, "source_ip": source_ip, "raw_log": line.strip(), "analysis": analysis})
                        processed += 1
                        break
    except Exception as e:
        log.error(f"Error reading Linux logs: {e}")
    ...
    try:
        ...
        while events and processed < 50:
            for event in events:
                ...
                threats_found.append({"event_type": event_type, "source_ip": source_ip, "raw_log": raw_log, "analysis": analysis})
                processed += 1
            if processed < 50:
                events = win32evtlog.ReadEventLog(hand, flags, 0)
    except ImportError:
        log.warning("pywin32 is not installed. Required for Windows event log parsing.")
    except Exception as e:
        log.error(f"Error reading Windows logs: {e}")
        ...
        with open(path, 'r', encoding="utf-8", errors="replace") as f:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    is_suspicious = any(p in path_req.lower() for p in honeypot.SENSITIVE_PATHS) or status_int >= 400
                    if is_suspicious:
                        ...
                        threats_found.append({"event_type": event_type, "source_ip": ip, "raw_log": raw_log, "analysis": analysis})
                        processed += 1
    except Exception as e:
        log.error(f"Error reading web logs: {e}")
...
def parse_logs() -> list[dict]:
    ...
    results = []
    if os_name == "Linux": results.extend(_parse_linux_logs())
    elif os_name == "Windows": results.extend(_parse_windows_logs())
    else: log.warning(f"Unsupported OS for log parsing: {os_name}")
    if config.WEB_LOG_PATH: results.extend(_parse_web_server_logs(config.WEB_LOG_PATH))
    return results
