...
def get_listening_ports() -> list[dict]:
    ...
    try:
        ...
        log.error(f"[ProcessMonitor] Error fetching connections: {e}")
...
def check_outbound_reputation() -> list[dict]:
    ...
    try:
        ...
        log.error(f"[ProcessMonitor] Error checking outbound: {e}")
...
def monitor_system() -> None:
    ...
    with db_init.get_connection() as conn:
        ...
        for anomaly in new_ports:
            ...
            log.info(f"[ProcessMonitor] ALERT {anomaly['severity']} - {anomaly['type']} on port {anomaly['port']}")
        ...
        for anomaly in outbound:
            ...
            """, (timestamp, anomaly["ip"], anomaly["type"], raw_log, anomaly["severity"],
                  ai_analysis["explanation"], ai_analysis["recommendation"], 0))
            log.info(f"[ProcessMonitor] {anomaly['severity']} - {anomaly['type']} to {anomaly['ip']}")
